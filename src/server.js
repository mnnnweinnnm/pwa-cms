...
<body>
  ...
  <script>
    ...
    async function loadCampaigns(){ const {campaigns}=await api('GET','/api/campaigns'); document.getElementById('camp-table-body').innerHTML=campaigns.map(c=>'<tr><td>'+esc(c.subdomain)+'</td><td>'+esc(c.pkgName)+'</td><td><span class="badge badge-'+esc(c.pkgLang)+'">'+esc(String(c.pkgLang).toUpperCase())+'</span></td><td>'+(c.downloadUrl?'<div class="link-box">'+esc(c.downloadUrl)+'</div>':'-')+'</td><td>'+(c.verified?'<span class="badge badge-verified">已驗證</span>':'<span class="badge badge-pending">待驗證</span>')+'</td><td><button class="btn btn-muted btn-sm" onclick="verifyCamp(\''+c.id+'\')">驗證</button> <button class="btn btn-danger btn-sm" onclick="delCamp(\''+c.id+'\')">刪除</button></td></tr>').join(''); }
    ...
    function domainActions(d){ return '<button class="btn btn-muted btn-sm" onclick="setDomainStatus(\''+d.domain+'\',\'active\')">啟用</button> <button class="btn btn-muted btn-sm" onclick="setDomainStatus(\''+d.domain+'\',\'pending\')">待處理</button> <button class="btn btn-muted btn-sm" onclick="setDomainStatus(\''+d.domain+'\',\'disabled\')">停用</button> <button class="btn btn-danger btn-sm" onclick="delDomain(\''+d.domain+'\')">刪除</button>'; }
    ...
    async function loadUsers(){ if(!isAdmin)return; const users=await api('GET','/api/users'); document.getElementById('user-table-body').innerHTML=users.map(u=>'<tr><td>'+esc(u.username)+'</td><td>'+esc(u.displayName)+'</td><td>'+esc(u.role)+'</td><td>'+(u.active?'<span class="badge badge-active">active</span>':'<span class="badge badge-disabled">disabled</span>')+'</td><td><button class="btn btn-muted btn-sm" onclick="toggleUser(\''+u.id+'\','+(!u.active)+')">'+(u.active?'停用':'啟用')+'</button> <button class="btn btn-danger btn-sm" onclick="delUser(\''+u.id+'\')">刪除</button></td></tr>').join(''); }
